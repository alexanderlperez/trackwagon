...
/**
 * Calculates minimum distance of a point from a great-circle path or its end points.
 * 
 * @param {LatLon} startPoint - Point denoting the start of the great-circle path
 * @param {LatLon} endPoint - Point denoting the end of the great-circle path
 * @param {Number} [precision=4] - no of significant digits to use for calculations and returned value
 * @return {Number} - minumum distance in km from third point to great-circle path
 */
LatLon.prototype.minimumCrossTrackDistance = function(startPoint, endPoint, precision){
    var ct = Math.abs(this.constrainedCrossTrackDistance(startPoint, endPoint, precision));
    var dAC = startPoint.distanceTo(this, 10);
    var dBC = endPoint.distanceTo(this, 10);
    return Math.min(ct, dAC, dBC);
}

/**
 * Returns a flag indicating if a point is within the bounding box defined by the two boundary points
 *
 * @param {LatLon} tl - Point denoting the top-left corner of a bounding box
 * @param {LatLon} br - Point denoting bottom-right corner of a bounding box
 * @returns {Boolean} Flag indicating if point is within bounds
 */
LatLon.prototype.isBounded = function(tl, br) {
    /* Check latitude bounds first. */
    if(tl.lat() >= this.lat() && this.lat() >= br.lat()){
            /* If your bounding box doesn't wrap the date line the valuemust be between the bounds.
               If your bounding box does wrap the date line it only needs to be higher than the left bound or lower than the right bound. */
        if(tl.lon() <= br.lon() && tl.lon() <= this.lon() && this.lon() <= br.lon()){
            return true;
        } else if(br.lon() > tl.lon() && (tl.lon() <= this.lon() && this.lon() <= br.lon())) {
            return true;
        }
    }
    return false;
...
/**
 * Given a pair of bearings in degrees, returns the difference in degrees between the two bearings 
 * 
 * @param {Number} bearingA - first bearing in degrees
 * @param {Number} bearingB - second bearing in degrees
 * @return {Number} - Difference between bearings in degrees
 */
Geo.differenceInBearings = function(bearingA, bearingB){
    var a = bearingA; 
    var b = bearingB;
